...
1. **Evidence ledger and mandatory citations.** Every scanner result and every file the agents read or search gets an ID (`SEC-001`, `READ-014`, ...). A finding must cite at least one ID. Findings that cite unknown IDs are dropped and counted in the report, so an invented claim cannot reach the page silently.
...
Requires Node 24+ and git. The agent is built on [`ensemble`](https://github.com/tSLoseth/ensemble), which is referenced as a sibling folder, so clone both side by side:

```bash
git clone https://github.com/tSLoseth/ensemble
git clone https://github.com/tSLoseth/tech-dd-agent
cd ensemble && npm install && cd ../tech-dd-agent
npm install
```
...
|---|---|---|---:|---|
| [board-pack-assistant](https://github.com/tSLoseth/board-pack-assistant) | My own full-stack Next.js app (AI board-pack briefing) | 68 / Red | 29 | [Markdown](examples/board-pack-assistant/report.md) · [HTML](examples/board-pack-assistant/report.html) |
| [tiny-agent-sdk](https://github.com/tSLoseth/tiny-agent-sdk) | My own small agent library (learning project) | 69 / Red | 30 | [Markdown](examples/tiny-agent-sdk/report.md) · [HTML](examples/tiny-agent-sdk/report.html) |
| [node-express-realworld-example-app](https://github.com/gothinkster/node-express-realworld-example-app) | Public reference Express + Prisma API | 42 / Red | 30 | [Markdown](examples/realworld-express/report.md) · [HTML](examples/realworld-express/report.html) |

All three are red. The two repos of mine are red because a single author wrote every commit and the team specialist rated that key-person risk critical, even though their numeric scores are in the amber range. For a personal project that rating is harsh, and it is exactly the kind of call a consultant would revisit. The public API is red on substance: a hard-coded fallback JWT signing secret (`process.env.JWT_SECRET || 'superSecret'`), which would let anyone who knows the default forge login tokens, plus a codebase with no commits in about 1,000 days.
...
| tiny-agent-sdk | 25 files, 1,094 lines | $0.18 |
| node-express-realworld-example-app | 67 files, 2,537 lines | $0.17 |

About $0.20 per repository. Offline mode is free.
...
- The model still makes judgement errors that grounding cannot catch: a finding can cite a real file and still misread it, or pick a severity a human would not. Every finding links to its evidence so a reviewer can check it quickly.
